...
struct Endpoint {
    id: ID,
    subgraph1_value: Option<String>,
}

impl Endpoint {
    async fn find_by_id(id: ID, _ctx: &Context<'_>) -> Self {
        Self {
            id: id.clone(),
            subgraph1_value: Some(format!("subgraph1-{}", id.as_str())),
        }
    }
}
...
#[Object]
impl Query {
    async fn subgraph1_query_value(&self, ctx: &Context<'_>) -> i32 {
...
#[Object]
impl Mutation {
    async fn subgraph1_increment_value(&self, ctx: &Context<'_>, by: Option<i32>) -> i32 {
        let storage = ctx.data::<Storage>().unwrap();
...
#[Subscription]
impl Subscription {
    async fn subgraph1_on_change_value(&self, ctx: &Context<'_>) -> impl Stream<Item = i32> {
        let sender = ctx.data::<CounterSender>().unwrap();
...
type Subgraph1Schema = Schema<Query, Mutation, Subscription>;

async fn graphql_handler(
    State(schema): State<Subgraph1Schema>,
    req: GraphQLRequest,
) -> GraphQLResponse {
...
async fn main() {
    ...
    println!("🦀 Subgraph-1 listening (HTTP+WS) on http://127.0.0.1:8082/graphql");
